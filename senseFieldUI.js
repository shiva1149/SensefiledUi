...
function ( $, qlik, cssBoot, csscheckBox,custom) {
	'use strict';
	var control = 0;
	...
	if(!$("style[id='bootstrap']").length > 0){
		...
		$( '<style>' ).html( custom ).appendTo( 'head' ); // Adding checkbox styles to head
...
/* 		snapshot: {
		...
		paint: function ( $element , layout) {
			...
			if(layout.InitialValue=='' || layout.InitialValue==undefined || layout.InitialValue=='undefined'){
				init="Select A Value";
			}else{
				init=layout.InitialValue;
			}
				...
				html += '			<select '+vars.dropDownStyle+' class="form-control" id="drpdwn" >';		
				html += '<option id="noval">'+init+'</option>';
					this.backendApi.eachDataRow(function(rownum, row) {
						if(row[0].qState==='S'){
							var active ='#52CC52'; var actv = 'selected'; var actcolor='#fff';
						}else{
							var active ='', actv = '';
						};
						if(row[0].qState==='X'){
						...
						console.log(vars.dimSelected);
						if(vars.oneSelected && row[0].qText === vars.dimSelected){
							elemNo = row[0].qElemNumber; 
						};			
						
						html += '<option  id='+n+'_'+row[0].qElemNumber+' style="background-color: '+active+'; color: '+actcolor+'; " '+actv+' '+dis+'>'+row[0].qText+'</option>';
